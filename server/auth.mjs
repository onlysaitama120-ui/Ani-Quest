 ...
 * No email verification: accounts are created verified and usable instantly.
 */
...
router.post('/signup', authLimit, async (req, res) => {
  ...
  const info = await db.run(
    `INSERT INTO users (email, pass_hash, pass_salt, email_verified, created_at)
     VALUES (?, ?, ?, 1, ?)`,
    [emailLower, hash, salt, Date.now()],
  );

  const user = await db.get('SELECT id, email, created_at FROM users WHERE id = ?', [info.lastInsertRowid]);
  await createSession(user.id, res); // auto-login after signup
